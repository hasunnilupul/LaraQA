<template>
  <button
    @click="toggle"
    type="button"
    title="Click to mark as favourite question"
    :class="classes"
  >
    <svg
      xmlns="http://www.w3.org/2000/svg"
      fill="currentColor"
      class="bi bi-star-fill w-7 h-7"
      viewBox="0 0 16 16"
    >
      <path
        d="M3.612 15.443c-.386.198-.824-.149-.746-.592l.83-4.73L.173 6.765c-.329-.314-.158-.888.283-.95l4.898-.696L7.538.792c.197-.39.73-.39.927 0l2.184 4.327 4.898.696c.441.062.612.636.282.95l-3.522 3.356.83 4.73c.078.443-.36.79-.746.592L8 13.187l-4.389 2.256z"
      />
    </svg>
    <span class="text-xs font-medium lining-nums font-serif">{{ count }}</span>
  </button>
</template>

<script>
export default {
  name: "favourite",
  props: ["question"],
  data() {
    return {
      isFavourited: this.question.is_favourited,
      count: this.question.favourites_count,
      id: this.question.id,
    };
  },
  computed: {
    classes() {
      return [
        "block mt-2 text-center",
        !this.signedIn
          ? "off"
          : this.isFavourited
          ? "favourite-active"
          : "favourite",
      ];
    },
    endpoint() {
      return `/questions/${this.id}/favourites`;
    },
    signedIn(){
        return window.Auth.signedIn;
    }
  },
  methods: {
    toggle() {
      if (!this.signedIn) {
        this.$toast.warning(
          "Please sign in to favourite this question",
          "Warning",
          { position: "topRight", timeout: 3000 }
        );
        return;
      }
      this.isFavourited ? this.destroy() : this.create();
    },
    create() {
      axios.post(this.endpoint).then((res) => {
        this.count++;
        this.isFavourited = true;
      });
    },
    destroy() {
      axios.delete(this.endpoint).then((res) => {
        this.count--;
        this.isFavourited = false;
      });
    },
  },
};
</script>